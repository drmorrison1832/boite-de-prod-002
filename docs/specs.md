...
- **Salariés** : données personnelles, coordonnées, informations sociales
- **Spectacles** : titre, code, numéro d'objet France Travail
...
- Ajout du champ `Salariés.dateAnniversaire` : date de fin du dossier d'intermittence en cours
- Si un contrat chevauche deux dossiers d'intermittence (i.e. sa période enjambe la `dateAnniversaire` d'un salarié), l'application affiche un avertissement — ce chevauchement peut être défavorable au salarié
